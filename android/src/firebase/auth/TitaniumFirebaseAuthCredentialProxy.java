...
public class TitaniumFirebaseAuthCredentialProxy extends KrollProxy {
	...
	// Constructor
	public TitaniumFirebaseAuthCredentialProxy(int provider,
			String accessToken, String secretToken, String providerID,
			String IDToken) {
		super();
		...
		case TitaniumFirebaseAuthModule.AUTH_PROVIDER_TYPE_TWITTER:
			mAuthCredential = TwitterAuthProvider.getCredential(accessToken,
					secretToken);
			break;
		case TitaniumFirebaseAuthModule.AUTH_PROVIDER_TYPE_GOOGLE:
			mAuthCredential = GoogleAuthProvider.getCredential(accessToken,
					secretToken);
			break;
		...
		case TitaniumFirebaseAuthModule.AUTH_PROVIDER_TYPE_PASSWORD:
			mAuthCredential = EmailAuthProvider.getCredential(accessToken,
					secretToken);
			break;
		case TitaniumFirebaseAuthModule.AUTH_PROVIDER_TYPE_PHONE:
			mAuthCredential = PhoneAuthProvider.getCredential(accessToken,
					secretToken);
			break;
		case TitaniumFirebaseAuthModule.AUTH_PROVIDER_TYPE_OAUTH: {
			mAuthCredential = OAuthProvider.getCredential(IDToken, accessToken,
					secretToken);
		}
			break;
		default:
